...
ScanDelayToWaitForMenusToOpenInMs := "48"
...
;========================Requirements========================
;-Windows 7 or later.
;-A browser extension that allow custom scripts like Tampermonkey for installing two other required user scripts. With a quick google search you should find the right version for your browser.
;-The Improved Building Menu script by steve and any script that disables anti aliasing (explanation follows).
;
;========================Installation========================
;-Install the other required user scripts with Tampermonkey (You can find links below in the "Links" section).
;-To run this hotkey script you need to download and install the AHK compiler.
;-After installation, you can simply run any AHK script by right clicking the .ahk file and selecting "Open" or "Run script".
...
;
;=========================Features===========================
;-All Unit commands and building menu entries can be assigned to any key or key combination. Simply change the character of any keybinding in the Hotkeys section. Changes are applied automatically when the file is saved. A complete list of possible keys can be found in the links section below. If you do not want certain hotkeys to be assigned, write "" after the equals sign
...
;-The hotkey script is currently not compatible with user scripts that hides certain entries in the building menu (i.e. Piperunner and Missiles). There may also be other user script which changes the look of the game which may not be compatible.
...
;-When the script simulates input and you give manual input at the same time, the mouse might twitch or cause the browser to scroll unexpectedly.
;
;==========================Links=============================
;-Improved Building Menu by steve: https://cdn.discordapp.com/attachments/945580784629215253/1206374723639054356/build-menu.js?ex=677bb843&is=677a66c3&hm=f6e17d634af81c9c6906d3eb8e409cfe06ec7bfd27df3e71507ab28da4ffee30&
;-To install click on the Tampermonkey extension -> "Create a new script..." -> paste the content of the link -> File -> Save. 
...
;-Add compatibility with the e hotkey (recognize menus that are opened by it).
;-After moving an occupied transport unit, the menu for unloading the occupants is currently not recognized.
;-In rare cases, a warning about the lack of improved building menus may incorrectly appear.
;-If the maximize script is used with your Windows taskbar set to hide automatically, it can sometimes cause the map to be cut off, affecting detection.
;
;=========================Changelog==========================
;2025-01-05 v0.001
...
CheckAndInstallUpdate(Notification)
{
	...
	if ScriptVersion < VerifiedMostRecentVersion
	{
		...
		if result = "yes"
		{
			...
			SettingsToSave := Array("Select", "Capture", "Wait", "Fire", "Infantry", "Mech", "Recon", "TCopter", "EndTurn", "CheckForUpdates", "PauseScript", "CloseScript", "SkipConfirmationMessageDelete", "SkipConfirmationMessageEndDay", "DebugMode", "UsePreciseSleepMode", "ScanDelayToWaitForMenusToOpenInMs")
			for n in SettingsToSave
			{
				ContentUpdatedVersion := StrReplace(ContentUpdatedVersion, "'" SettingsToSave[A_index] ' :="', "'" SettingsToSave[A_index] ' := "')
...
ImageFiles := Array("55w41h.png", "Capture.png", "Delete.png", "Fire.png", "Hide.png", "Supply.png", "Unload.png", "Wait.png", "EndTurn.png", "Confirm.png", "220w1h.png", "1w220h.png", "90w10hBL.png", "48w1hGray.png", "Coin.png", "CoinMarked.png", "Calculator.png", "CalculatorMarked.png")
...
for in ImageFiles
{
	if !FileExist("images\" ImageFiles[A_index]) or FileGetSize("images\" ImageFiles[A_index]) < 24
	{
		...
		if FileGetSize("images\" ImageFiles[A_index]) < 24
			msgbox "Failed to download the required image adjustment files.`nPlease try again later.", "AWBW Advanced Hotkeys"
	}
}
...
Hotif
	...
	Try Hotkey CloseScript, CloseScriptPressed
...
#Hotif WinActive("View Game - AWBW") && SimulatingInputsNow == "true"
*LButton::return
#Hotif 
{
}
...
CheckIfSthIsSelected(TypeOfSearch)
{
	...
	{
		if MostRecentError = "Improved building menu missing?"
			return
		...
		Msgbox "It looks like the improved build menu mod is not installed, which is essential for the hotkeys to work properly. If this has been installed, it is a false positive.", "AWBW Advanced Hotkeys - Warning"
		;Tooltip "It looks like the improved build menu mod is not installed, which is essential for the hotkeys to work properly. If this has been installed, it is a false positive.", MapX + MapW / 2, MapY + MapH / 2, 2
		return
	}
	...
	if InGameMenuW > InGameMenuH * 1.5
		return
...
EndTurnPressed(ThisHotkey)
{
	...
	MouseGetPos &mouseX, &mouseY
	if isEndDayConfirmationBoxOpen = "true"
	{
		if InGameMenuType != 0
		{
			DeselectMenu()
	...
	if isGameMaximized = "false"
	{
		if Scan.ImageRegion("images\EndTurn.png", HalfScreenW + 100, 50, HalfScreenW - 100, MapY - 50, variance:=0, &returnX:=0, &returnY:=0, centerResults:=0, "RLBT")
		{
			Click returnX, returnY
			mousemove mouseX, mouseY
			isEndDayConfirmationBoxOpen := "true"
		}
		else
		{
	...
	if SkipConfirmationMessageEndDay = "true"
	{
		PreciseSleep()
		Scan.Update()
		if Scan.ImageRegion("images\Confirm.png", MapX, MapY, MapW, MapH, variance:=0, &returnX:=0, &returnY:=0, centerResults:=0, "TBLR")
		...
		{
			MostRecentError := "Cannot find the confirmation box. [3]"
			return
		}
	}
}
...
BuyUnitIfAffordable(MenuItemX, MenuItemY)
{
	global
	Critical
	if Scan.PixelRegion(816, MenuItemX, MenuItemY, round(16 / 100 * Zoom), round(16 / 100 * Zoom), variance:=0, &returnX:=0, &returnY:=0, "TBLR")	;10 muss wieder weg
	{
...
{
	global
	if isUnitSelected = "true" and (Thishotkey = Wait or Thishotkey = Capture or ThisHotkey = Fire)
	{
		if ThisHotkey = Fire
		{
			...
			PreciseSleep()
			Scan.Update()
			if Scan.PixelRegion(816, mouseXUnitCommand - 75, mouseYUnitCommand, 150, MapY - mouseYUnitCommand, variance:=0, &returnX:=0, &returnY:=0, "LRBT")
			{
				Click mouseXUnitCommand, mouseYUnitCommand
				isEnemyLockedIn := "true"
			}
			else
				isEnemyLockedIn := "false"
			;msgbox isEnemyLockedIn
		...
		if  isUnitSelected = "true" and wasADifferentUnitSelected = "true" or isUnitSelected = "false" and InGameMenuType != "Unit" or ThisHotkey = Fire and InGameMenuType = "Unit" ;InGameMenuType = "Base" or  überflüssig?
		{
			
			if ThisHotkey = Fire and isEnemyLockedIn = "true"
				return
			MouseGetPos &mouseX, &mouseY
			DeselectMenu()
	...
	if InGameMenuType = "Unit"
	{
		...
		if ThisHotkey = Capture and isCaptureAvailable = "true"
			...
			Click WaitX, WaitY
		else if ThisHotkey = Fire and isFireAvailable = "true"
			Click FireX, FireY
		else
		{
			DeselectMenu()
...
CheckIfMapHasChanged()
{
	...
	Scan.Update()
	if Scan.PixelRegion(16711680, round(MapX + MapW / 2) - 100, MapY, round(MapX + MapW / 2), round(MapH / 2), 0, &returnX:=0, &returnY:=0, "BTLR")
	{
		CalibrationCountdown := 0
		Suspend "1"
		return
	...
	if !Scan.PixelPosition(12226835, CoinX, CoinY,variance:=0) and !Scan.PixelPosition(16701507, CoinX + 1, CoinY,variance:=0) and !Scan.PixelPosition(2445730, CoinX, CoinY,variance:=0) and !Scan.PixelPosition(3366827, CoinX + 1, CoinY,variance:=0)and (VerifyMapPixel1 != Scan.GetPixel(MapX + 1, MapY + 1, 0) or VerifyMapPixel2 != Scan.GetPixel(MapX + 2, MapY + 1, 0) or VerifyMapPixel3 != Scan.GetPixel(MapX + 1, MapY + 2, 0))
	{
		isMapCalibrated := "false"
...
CalibrateMapCoordAndZoom()
{
	...
	VerifyMapPixel1 := Scan.GetPixel(MapX + 1, MapY + 1, 0)
	VerifyMapPixel2 := Scan.GetPixel(MapX + 2, MapY + 1, 0)
	VerifyMapPixel3 := Scan.GetPixel(MapX + 1, MapY + 2, 0)
	searchX := MapX + 1
	LastGrasFoundX := 0
	NumberOfTilesX := 0
	NumberOfTilesY := 0
	...
	while Scan.PixelRegion(12640304, searchX, MapY + 1, MapX + MapW - searchX, MapH - 10, 0, &returnX:=0, &returnY:=0, "TBLR")
	{
		searchX++
		GrasAmount := Scan.PixelCountRegion(12640304, searchX, MapY + 1, 1, MapH,variance:=0)
		LightGrassAmount := Scan.PixelCountRegion(14215200, searchX, MapY + 1, 1, MapH,variance:=0)
		if GrasAmount > MapH / 10 and GrasAmount > LightGrassAmount * 10
		{
			Counter++
			if LastGrasFoundX = 0
			...
			{
				if Scan.PixelCountRegion(14215200, LastGrasFoundX + 14, MapY + 1, 1, MapH,variance:=0) > 0 and Scan.PixelCountRegion(14215200, searchX - 1, MapY + 1, 1, MapH,variance:=0) > 0
				{
...
SearchUnoccupiedTerrain()
{
	...
	if CloseMenuTile != 0
	{
		if Scan.PixelRegion(0, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or  Scan.PixelRegion(7891064, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or Scan.PixelRegion(8153720, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or Scan.PixelRegion(16316528, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or Scan.PixelRegion(1446418, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or Scan.PixelRegion(328965, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") or Scan.PixelRegion(393216, MapX + 1, MapY + 1 + ((CloseMenuTile - 1 ) * TileSize), TileSize - 2, TileSize - 2, 0, &returnX:=0, &returnY:=0, "LRTB") ;8153720: Orange star, Blue Moon Green Earth, 1446418: Noir Eclipse, 7891064: rest 
		{
		CloseMenuTile := 0
		}
	}
	else
	{
		Loop NumberOfTilesY
		{
			...
			{
				CloseMenuTile := A_index
				CloseMenuX := MapX + (TileSize / 2)
				CloseMenuY := MapY + (TileSize / 2) + ((A_index - 1) * TileSize)
				break
			}	
		}
	}
